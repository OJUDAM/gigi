...
public class MybatisGigiConfiguration {
    ...
    public SqlSessionFactory sqlSessionFactory(@Qualifier("primaryDataSource")DataSource primaryDataSource, ApplicationContext applicationContext) throws Exception {
        ...
        sqlSession.setConfigLocation(applicationContext.getResource("classpath:primary/mybatis-config.xml")); //mybatis-config.xml의 경로
        sqlSession.setMapperLocations(applicationContext.getResources("classpath:primary/mapper/*.xml")); //쿼리문을 관리하는 mapper파일의 경로
